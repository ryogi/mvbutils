...
"build.pkg.binary" <-
function( pkg, character.only=FALSE, flags=character(0), cull.old.builds=TRUE, multiarch=FALSE){
  i <- 1
  repeat{ 
  ...
  block <- function( do.subs=TRUE, bs17=FALSE, blank.stop=FALSE, auto.link=FALSE, Rd2.Rlike=FALSE,
    ...
    #############
      ...
      if( Rd2.Rlike) {
        ...
        block <- make.Rd2( block, width=width)
      }
...
function( libname, pkgname){
   ...
   for( idll in dlls) {
     ...
     this.dll.info <- library.dynam( dll.name, package=pkgname, lib.loc=libname)
...
"install.pkg" <-
function( pkg, character.only=FALSE, lib=.libPaths()[1], flags=character(0)){
  set.pkg.and.dir( FALSE)  
  rcmdgeneric.pkg2( pkg, '', indir=sourcedir, outdir='.', cmd='INSTALL',  
      flags=c( '--no-multiarch', flags, '-l ' %&% shQuote( lib)))    
}
